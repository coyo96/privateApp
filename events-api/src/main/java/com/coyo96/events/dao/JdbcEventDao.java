...
public class JdbcEventDao implements EventDao {
    ...
    public int createNewEvent(Event event) {
        ...
        try {
            return jdbcTemplate.update(sql, int.class, event.getOrganizerId(), event.getEventName(),
                    event.getEventTime(), event.getDescription(), event.getImgUrl(), event.getTicketCost(),
                    event.getPurchaseTicketUrl(), event.getIsPrivate());
        } catch (CannotGetJdbcConnectionException e) {
            log.error(e.getMessage());
            throw new DaoException("An error occurred connecting to the database.");
        }
    ...
    private Event mapRowSetToEvent(SqlRowSet rowSet) {
        ...
        long organizerId = rowSet.getLong("organizer_id");
